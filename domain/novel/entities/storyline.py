...
class Storyline(BaseEntity):
    ...
    def __init__(
        ...
        - storyline_type: `StorylineType` 类型的业务输入，参与本函数的主要计算或流程控制。
        - status: `StorylineStatus` 类型的业务输入，参与本函数的主要计算或流程控制。
        - estimated_chapter_start: `int` 类型的业务输入，参与本函数的主要计算或流程控制。
        - estimated_chapter_end: `int` 类型的业务输入，参与本函数的主要计算或流程控制。
        - milestones: `Optional[List[StorylineMilestone]]` 类型的业务输入，参与本函数的主要计算或流程控制。
        - current_milestone_index: `int` 类型的业务输入，参与本函数的主要计算或流程控制。
        - name: 业务标识或配置键，用于定位目标记录。
        - description: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        - last_active_chapter: `int` 类型的业务输入，参与本函数的主要计算或流程控制。
        - progress_summary: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
