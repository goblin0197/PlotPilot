...
class ForeshadowingRegistry(BaseEntity):
    ...
    def _clamp_foreshadowing_chapters(f: Foreshadowing) -> Foreshadowing:
        """同步执行 `_clamp_foreshadowing_chapters` 对应的业务步骤。

        职责: 位于领域层，负责上述行为，并把相关输入、输出和副作用集中在 `_clamp_foreshadowing_chapters` 这一入口。
        关键输入:
        - f: `Foreshadowing` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `Foreshadowing`；具体语义由调用场景和返回类型共同约束。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
