...
class AutopilotDaemon:
    ...
    def __init__(
        ...
        - context_builder: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - background_task_service: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - planning_service: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - story_node_repo: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - chapter_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - poll_interval: `int` 类型的业务输入，参与本函数的主要计算或流程控制。
        - voice_drift_service: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - circuit_breaker: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - chapter_workflow: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - aftermath_pipeline: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - volume_summary_service: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - foreshadowing_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - knowledge_service: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
...
"""
    ...
    def _get_last_macro_word_anchor(self, novel_id: str) -> int:
        """同步执行 `_get_last_macro_word_anchor` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_get_last_macro_word_anchor` 这一入口。
        关键输入:
    ...
    async def _stream_llm_with_stop_watch(
        ...
        if novel is not None:
            ...
            async def _watch_stop_from_db() -> None:
                """异步执行 `_watch_stop_from_db` 对应的业务步骤。

                职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_watch_stop_from_db` 这一入口。
