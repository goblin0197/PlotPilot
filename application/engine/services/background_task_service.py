...
class BackgroundTask:
    ...
    def __init__(
        ...
        关键输入:
        - task_id: 业务标识或配置键，用于定位目标记录。
        - task_type: `TaskType` 类型的业务输入，参与本函数的主要计算或流程控制。
...
class BackgroundTaskService:
    ...
    def __init__(
        ...
        - chapter_indexing_service: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - storyline_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - chapter_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - plot_arc_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - narrative_event_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        关键输出: 无返回值；初始化结果体现在当前实例的字段状态上。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
