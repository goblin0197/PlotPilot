...
def init_streaming_bus() -> mp.Queue:
    """同步执行 `init_streaming_bus` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `init_streaming_bus` 这一入口。
    关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
    关键输出: `mp.Queue`；具体语义由调用场景和返回类型共同约束。
...
def inject_stream_queue(queue: mp.Queue):
    """同步执行 `inject_stream_queue` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `inject_stream_queue` 这一入口。
    关键输入:
    - queue: `mp.Queue` 类型的业务输入，参与本函数的主要计算或流程控制。
...
def _get_queue() -> Optional[mp.Queue]:
    """同步执行 `_get_queue` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_get_queue` 这一入口。
    关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
    关键输出: `Optional[mp.Queue]`；当目标不存在、依赖不可用或条件不满足时可能返回 `None`。
    副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
    异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    """
...
class StreamingBus:
    ...
    def __init__(self, queue: Optional[mp.Queue] = None):
        ...
        关键输入:
        - queue: `Optional[mp.Queue]` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def publish(self, novel_id: str, chunk: str):
        """同步执行 `publish` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `publish` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - chunk: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
    ...
    def subscribe(self, novel_id: str) -> asyncio.Queue:
        """同步执行 `subscribe` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `subscribe` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        关键输出: `asyncio.Queue`；具体语义由调用场景和返回类型共同约束。
    ...
    def unsubscribe(self, novel_id: str, queue: asyncio.Queue):
        """同步执行 `unsubscribe` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `unsubscribe` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - queue: `asyncio.Queue` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 可能访问文件系统。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
    ...
    def get_chunk(self, novel_id: str, timeout: float = 0.05) -> Optional[str]:
        ...
        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `get_chunk` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - timeout: `float` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    async def get_chunk_async(self, novel_id: str, timeout: float = 0.05) -> Optional[str]:
        """异步读取 `chunk_async` 相关的数据或状态。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `get_chunk_async` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - timeout: `float` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `Optional[str]`；当目标不存在、依赖不可用或条件不满足时可能返回 `None`。
    ...
    def get_chunk_non_blocking(self, novel_id: str) -> Optional[str]:
        """同步读取 `chunk_non_blocking` 相关的数据或状态。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `get_chunk_non_blocking` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        关键输出: `Optional[str]`；当目标不存在、依赖不可用或条件不满足时可能返回 `None`。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
    ...
    def clear(self, novel_id: str):
        ...
        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `clear` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
