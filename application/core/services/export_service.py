...
def _safe_filename_stem(title: str, max_len: int = 80) -> str:
    """同步执行 `_safe_filename_stem` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_safe_filename_stem` 这一入口。
    关键输入:
...
def _novel_id_str(novel: Novel) -> str:
    """同步执行 `_novel_id_str` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_novel_id_str` 这一入口。
    关键输入:
    - novel: `Novel` 类型的业务输入，参与本函数的主要计算或流程控制。
    关键输出: `str`；具体语义由调用场景和返回类型共同约束。
    副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
    异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    """
...
def _chapter_display_title(ch: Chapter) -> str:
    """同步执行 `_chapter_display_title` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_chapter_display_title` 这一入口。
    关键输入:
    - ch: `Chapter` 类型的业务输入，参与本函数的主要计算或流程控制。
    关键输出: `str`；具体语义由调用场景和返回类型共同约束。
    副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
    异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    """
...
def _content_to_html_paragraphs(text: str) -> str:
    """同步执行 `_content_to_html_paragraphs` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_content_to_html_paragraphs` 这一入口。
    关键输入:
...
def _cjk_font_paths() -> Iterator[Path]:
    """同步执行 `_cjk_font_paths` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_cjk_font_paths` 这一入口。
    关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
    关键输出: `Iterator[Path]`；具体语义由调用场景和返回类型共同约束。
    副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
    异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    """
...
class ExportService:
    ...
    def __init__(self, novel_repository: NovelRepository, chapter_repository: ChapterRepository):
        ...
        关键输入:
        - novel_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
    ...
    def export_novel(self, novel_id: str, format: str) -> Tuple[bytes, str, str]:
        """同步执行 `export_novel` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `export_novel` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - format: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `Tuple[bytes, str, str]`；按调用约定排序或过滤后的结果集合。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 会显式抛出 `ValueError, except`；底层依赖的数据库、文件、网络或校验异常会按调用栈透传。
        """
        try:
    ...
    def export_chapter(self, chapter_id: str, format: str) -> Tuple[bytes, str, str]:
        """同步执行 `export_chapter` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `export_chapter` 这一入口。
        关键输入:
    ...
    def _export_to_epub(self, novel: Novel, chapters: list[Chapter]) -> Tuple[bytes, str, str]:
        """同步执行 `_export_to_epub` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_export_to_epub` 这一入口。
        关键输入:
        - novel: `Novel` 类型的业务输入，参与本函数的主要计算或流程控制。
        - chapters: `list[Chapter]` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `Tuple[bytes, str, str]`；按调用约定排序或过滤后的结果集合。
        副作用: 可能访问文件系统。
...
{body}
    ...
    def _try_register_cjk_font(self, pdf) -> bool:
        """同步执行 `_try_register_cjk_font` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_try_register_cjk_font` 这一入口。
        关键输入:
        - pdf: 业务输入参数，参与本函数的主要计算或流程控制。
    ...
    def _export_to_pdf(self, novel: Novel, chapters: list[Chapter]) -> Tuple[bytes, str, str]:
        """同步执行 `_export_to_pdf` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_export_to_pdf` 这一入口。
        关键输入:
        - novel: `Novel` 类型的业务输入，参与本函数的主要计算或流程控制。
        - chapters: `list[Chapter]` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `Tuple[bytes, str, str]`；按调用约定排序或过滤后的结果集合。
        ...
        def add_text(size: float, text: str, line_h: float) -> None:
            """同步执行 `add_text` 对应的业务步骤。

            职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `add_text` 这一入口。
            关键输入:
            - size: 数量、范围或分页控制参数，用于限制处理规模。
            - text: 文本或提示词内容，是生成、解析或展示逻辑的主要输入。
            - line_h: `float` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _export_to_docx(self, novel: Novel, chapters: list[Chapter]) -> Tuple[bytes, str, str]:
        """同步执行 `_export_to_docx` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_export_to_docx` 这一入口。
        关键输入:
        - novel: `Novel` 类型的业务输入，参与本函数的主要计算或流程控制。
        - chapters: `list[Chapter]` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `Tuple[bytes, str, str]`；按调用约定排序或过滤后的结果集合。
    ...
    def _export_to_markdown(self, novel: Novel, chapters: list[Chapter]) -> Tuple[bytes, str, str]:
        """同步执行 `_export_to_markdown` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_export_to_markdown` 这一入口。
        关键输入:
        - novel: `Novel` 类型的业务输入，参与本函数的主要计算或流程控制。
        - chapters: `list[Chapter]` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `Tuple[bytes, str, str]`；按调用约定排序或过滤后的结果集合。
