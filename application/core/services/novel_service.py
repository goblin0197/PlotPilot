...
class NovelService:
    ...
    def get_novel(self, novel_id: str) -> Optional[NovelDTO]:
        ...
        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `get_novel` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        关键输出: `Optional[NovelDTO]`；当目标不存在、依赖不可用或条件不满足时可能返回 `None`。
    ...
    def _check_has_bible(self, novel_id: str) -> bool:
        """同步执行 `_check_has_bible` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_check_has_bible` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
    ...
    def _check_has_outline(self, novel_id: str) -> bool:
        """同步执行 `_check_has_outline` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_check_has_outline` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        关键输出: 布尔值，表示判断、校验或执行条件是否成立。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
