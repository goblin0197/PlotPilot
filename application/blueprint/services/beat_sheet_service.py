...
class BeatSheetService:
    ...
    def __init__(
        ...
        关键输入:
        - beat_sheet_repo: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
    ...
    def _deduplicate_and_limit_tokens(self, context: Dict, max_tokens: int) -> Dict:
        ...
        def estimate_tokens(text: str) -> int:
            """同步执行 `estimate_tokens` 对应的业务步骤。

            职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `estimate_tokens` 这一入口。
            关键输入:
        ...
        def deduplicate_list(items: List[Dict], key: str = "description") -> List[Dict]:
            """同步执行 `deduplicate_list` 对应的业务步骤。

            职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `deduplicate_list` 这一入口。
            关键输入:
