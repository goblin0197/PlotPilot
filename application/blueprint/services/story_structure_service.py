...
class StoryStructureService:
    ...
    def __init__(
        ...
        - chapter_renumber_coordinator: `Optional['ChapterRenumberCoordinator']` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _enrich_chapter_nodes_from_chapters_table(
        ...
        def walk(ns: List[Dict[str, Any]]) -> None:
            ...
            职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `walk` 这一入口。
            关键输入:
            - ns: `List[Dict[str, Any]]` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _enrich_flat_chapter_nodes(self, novel_id: str, nodes: List[Dict[str, Any]]) -> None:
        """同步执行 `_enrich_flat_chapter_nodes` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_enrich_flat_chapter_nodes` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - nodes: `List[Dict[str, Any]]` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
