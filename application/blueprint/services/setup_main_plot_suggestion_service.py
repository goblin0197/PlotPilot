...
class SetupMainPlotSuggestionService:
    """定义 `SetupMainPlotSuggestionService`，作为应用层中的服务对象。

    职责: 组织多个协作者完成一段用例、领域流程或基础设施适配。
    ...
    def _build_context(self, novel_id: str) -> Dict[str, Any]:
        """同步执行 `_build_context` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_build_context` 这一入口。
        关键输入:
    ...
    def _parse_plot_json(raw: str) -> List[Dict[str, Any]]:
        """同步执行 `_parse_plot_json` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_parse_plot_json` 这一入口。
        关键输入:
    ...
    def _normalize_options(raw_list: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        """同步执行 `_normalize_options` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_normalize_options` 这一入口。
        关键输入:
        - raw_list: `List[Dict[str, Any]]` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _fallback_options(self, ctx: Dict[str, Any]) -> List[Dict[str, str]]:
        """同步执行 `_fallback_options` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_fallback_options` 这一入口。
        关键输入:
        - ctx: `Dict[str, Any]` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `List[Dict[str, str]]`；按调用约定排序或过滤后的结果集合。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
    ...
    async def suggest_options(self, novel_id: str) -> List[Dict[str, str]]:
        """异步执行 `suggest_options` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `suggest_options` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        关键输出: `List[Dict[str, str]]`；按调用约定排序或过滤后的结果集合。
