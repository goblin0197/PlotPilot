...
def _sanitize_llm_json_output(raw: str) -> str:
    """同步执行 `_sanitize_llm_json_output` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_sanitize_llm_json_output` 这一入口。
    关键输入:
...
def _extract_outer_json_value(text: str) -> str:
    """同步执行 `_extract_outer_json_value` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_extract_outer_json_value` 这一入口。
    关键输入:
...
def _repair_json_string(text: str) -> str:
    """同步执行 `_repair_json_string` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_repair_json_string` 这一入口。
    关键输入:
    - text: 文本或提示词内容，是生成、解析或展示逻辑的主要输入。
    关键输出: `str`；具体语义由调用场景和返回类型共同约束。
    副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
    ...
    def _close_json(s: str) -> str:
        """同步执行 `_close_json` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_close_json` 这一入口。
        关键输入:
        - s: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `str`；具体语义由调用场景和返回类型共同约束。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
...
def get_macro_plan_progress(novel_id: str) -> Dict:
    """同步读取 `macro_plan_progress` 相关的数据或状态。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `get_macro_plan_progress` 这一入口。
    关键输入:
...
def get_macro_plan_result(novel_id: str) -> Dict:
    """同步读取 `macro_plan_result` 相关的数据或状态。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `get_macro_plan_result` 这一入口。
    关键输入:
    - novel_id: 业务标识或配置键，用于定位目标记录。
    关键输出: `Dict`；包含后续流程所需字段的结构化映射。
    副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
...
class ContinuousPlanningService:
    ...
    def __init__(
        ...
        关键输入:
        - story_node_repo: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - chapter_element_repo: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - llm_service: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - bible_service: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - chapter_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
    ...
    def _apply_precise_node_update(self, node: Dict, update: Dict) -> None:
        """同步执行 `_apply_precise_node_update` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_apply_precise_node_update` 这一入口。
        关键输入:
        - node: `Dict` 类型的业务输入，参与本函数的主要计算或流程控制。
        - update: `Dict` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _calculate_precise_max_tokens(self, structure_preference: Dict[str, int]) -> int:
        """同步执行 `_calculate_precise_max_tokens` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_calculate_precise_max_tokens` 这一入口。
        关键输入:
        - structure_preference: `Dict[str, int]` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _calculate_precise_repair_max_tokens(self, incomplete_acts: List[Dict]) -> int:
        """同步执行 `_calculate_precise_repair_max_tokens` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_calculate_precise_repair_max_tokens` 这一入口。
        关键输入:
        - incomplete_acts: `List[Dict]` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `int`；具体语义由调用场景和返回类型共同约束。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
    ...
    def _find_incomplete_precise_acts(self, skeleton: Dict) -> List[Dict]:
        """同步执行 `_find_incomplete_precise_acts` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_find_incomplete_precise_acts` 这一入口。
        关键输入:
        - skeleton: `Dict` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _get_total_volumes(self, structure_preference: Optional[Dict[str, int]]) -> int:
        """同步执行 `_get_total_volumes` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_get_total_volumes` 这一入口。
        关键输入:
        - structure_preference: `Optional[Dict[str, int]]` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `int`；具体语义由调用场景和返回类型共同约束。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
    ...
    def _update_macro_progress(
        ...
        *,
        ...
        """同步执行 `_update_macro_progress` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_update_macro_progress` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - status: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        - current: `Optional[int]` 类型的业务输入，参与本函数的主要计算或流程控制。
        - total: `Optional[int]` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def initialize_macro_plan_task(self, novel_id: str) -> None:
        """同步执行 `initialize_macro_plan_task` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `initialize_macro_plan_task` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
    ...
    def store_macro_plan_result(self, novel_id: str, result: Dict) -> None:
        """同步执行 `store_macro_plan_result` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `store_macro_plan_result` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - result: `Dict` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
    ...
    def store_macro_plan_error(self, novel_id: str, error: str) -> None:
        """同步执行 `store_macro_plan_error` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `store_macro_plan_error` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - error: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
