...
def _map_location_kind(raw_type: str) -> str:
    """同步执行 `_map_location_kind` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_map_location_kind` 这一入口。
    关键输入:
    - raw_type: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
...
def stable_containment_triple_id(novel_id: str, bible_location_id: str) -> str:
    """同步执行 `stable_containment_triple_id` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `stable_containment_triple_id` 这一入口。
    关键输入:
...
def stable_anchor_triple_id(novel_id: str, bible_location_id: str) -> str:
    """同步执行 `stable_anchor_triple_id` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `stable_anchor_triple_id` 这一入口。
    关键输入:
    - novel_id: 业务标识或配置键，用于定位目标记录。
    - bible_location_id: 业务标识或配置键，用于定位目标记录。
    关键输出: `str`；具体语义由调用场景和返回类型共同约束。
...
class BibleLocationTripleSyncService:
    ...
    def sync_from_locations(self, novel_id: str, locations: List[Dict[str, Any]]) -> None:
        """同步执行 `sync_from_locations` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `sync_from_locations` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - locations: `List[Dict[str, Any]]` 类型的业务输入，参与本函数的主要计算或流程控制。
