...
class BibleService:
    ...
    def _validate_locations_forest(self, locations: list) -> None:
        """校验 `locations_forest` 的输入边界并返回规范化结果。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_validate_locations_forest` 这一入口。
        关键输入:
        - locations: `list` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _sync_location_triples(self, novel_id: str, bible: Bible) -> None:
        """同步执行 `_sync_location_triples` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_sync_location_triples` 这一入口。
        关键输入:
