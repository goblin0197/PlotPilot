...
def _extract_outer_json_object(text: str) -> str:
    """同步执行 `_extract_outer_json_object` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_extract_outer_json_object` 这一入口。
    关键输入:
...
def _parse_llm_json_to_dict(raw: str) -> Dict[str, Any]:
    """同步执行 `_parse_llm_json_to_dict` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_parse_llm_json_to_dict` 这一入口。
    关键输入:
    - raw: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
    关键输出: `Dict[str, Any]`；包含后续流程所需字段的结构化映射。
    副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
    异常/边界: 会显式抛出 `json`；底层依赖的数据库、文件、网络或校验异常会按调用栈透传。
    """
...
def _default_location_importance(_loc_data: Dict[str, Any]) -> str:
    """同步执行 `_default_location_importance` 对应的业务步骤。

    职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_default_location_importance` 这一入口。
    关键输入:
    - _loc_data: 结构化数据载荷，通常来自请求、数据库行或上游服务。
    关键输出: `str`；具体语义由调用场景和返回类型共同约束。
    副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
    异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    """
...
class AutoBibleGenerator:
    ...
    def __init__(self, llm_service: LLMService, bible_service: BibleService, worldbuilding_service: WorldbuildingService = None, triple_repository: TripleRepository = None):
        ...
        - worldbuilding_service: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - triple_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
...
```"""
    ...
    def _worldbuilding_dict_nonempty(self, data: Dict[str, Any]) -> bool:
        """同步执行 `_worldbuilding_dict_nonempty` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_worldbuilding_dict_nonempty` 这一入口。
        关键输入:
