...
class ConsistencyIssue:
    ...
    def __init__(
        ...
        关键输入:
        - issue_type: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        - severity: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        - description: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        - location: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        - suggestion: `Optional[str]` 类型的业务输入，参与本函数的主要计算或流程控制。
...
class ChapterReviewResult:
    ...
    def __init__(
        ...
        - issues: `List[ConsistencyIssue]` 类型的业务输入，参与本函数的主要计算或流程控制。
        - overall_score: `float` 类型的业务输入，参与本函数的主要计算或流程控制。
        - improvement_suggestions: `List[str]` 类型的业务输入，参与本函数的主要计算或流程控制。
        - reviewed_at: `datetime` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；初始化结果体现在当前实例的字段状态上。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
...
class ChapterReviewService:
    ...
    def __init__(
        ...
        - cast_repo: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - timeline_repo: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - storyline_repo: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - foreshadowing_repo: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - vector_store: `'ChromaDBVectorStore'` 类型的业务输入，参与本函数的主要计算或流程控制。
