...
class SingleReaderFeedbackPayload(BaseModel):
    ...
    def normalize_persona(cls, value: object) -> str:
        """校验 `persona` 的输入边界并返回规范化结果。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `normalize_persona` 这一入口。
        关键输入:
...
class ReaderSimulationLlmPayload(BaseModel):
    ...
    def clamp_readability(cls, value: object) -> float:
        """校验 `clamp_readability` 的输入边界并返回规范化结果。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `clamp_readability` 这一入口。
        关键输入:
        - value: 待校验、转换或写入的值，类型约束为 `object`。
    ...
    def normalize_hook(cls, value: object) -> str:
        """校验 `hook` 的输入边界并返回规范化结果。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `normalize_hook` 这一入口。
        关键输入:
        - value: 待校验、转换或写入的值，类型约束为 `object`。
        关键输出: `str`；具体语义由调用场景和返回类型共同约束。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
