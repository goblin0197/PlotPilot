...
class ChapterRenumberExtension(Protocol):
    ...
    def name(self) -> str:
        ...
        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `name` 这一入口。
    ...
    def apply(self, spec: ChapterRenumberSpec, ctx: ChapterRenumberContext) -> None:
        ...
        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `apply` 这一入口。
        关键输入:
        - spec: `ChapterRenumberSpec` 类型的业务输入，参与本函数的主要计算或流程控制。
        - ctx: `ChapterRenumberContext` 类型的业务输入，参与本函数的主要计算或流程控制。
...
class ForeshadowRegistryChapterRenumberExtension:
    """定义 `ForeshadowRegistryChapterRenumberExtension`，作为应用层中的业务对象。
...
class VectorStoreChapterRenumberExtension:
    """定义 `VectorStoreChapterRenumberExtension`，作为应用层中的业务对象。
...
class ChapterRenumberCoordinator:
    ...
    def __init__(
        ...
        关键输入:
        - context: `ChapterRenumberContext` 类型的业务输入，参与本函数的主要计算或流程控制。
        - extensions: `Sequence[ChapterRenumberExtension]` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def on_chapter_deleted(self, novel_id: str, deleted_chapter_number: int) -> None:
        """同步执行 `on_chapter_deleted` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `on_chapter_deleted` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - deleted_chapter_number: `int` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
