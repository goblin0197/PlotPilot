...
class TensionAnalyzer:
    ...
    def __init__(
        ...
        - llm_client: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - chapter_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
        - plot_arc_repository: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
    ...
    async def analyze_tension(self, request: TensionSlingshotRequest) -> TensionDiagnosis:
        """异步执行 `tension` 对应的分析、生成或评估流程。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `analyze_tension` 这一入口。
        关键输入:
        - request: `TensionSlingshotRequest` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `TensionDiagnosis`；具体语义由调用场景和返回类型共同约束。
    ...
    def _analyze_statistics(self, events: List[dict], target_chapter: int) -> Dict:
        """同步执行 `_analyze_statistics` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_analyze_statistics` 这一入口。
        关键输入:
        - events: `List[dict]` 类型的业务输入，参与本函数的主要计算或流程控制。
        - target_chapter: `int` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _build_prompt(
        ...
        """同步执行 `_build_prompt` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_build_prompt` 这一入口。
        关键输入:
        - events: `List[dict]` 类型的业务输入，参与本函数的主要计算或流程控制。
        - stats: `Dict` 类型的业务输入，参与本函数的主要计算或流程控制。
        - request: `TensionSlingshotRequest` 类型的业务输入，参与本函数的主要计算或流程控制。
        - repository_context: 外部协作者实例，负责仓储、服务编排或第三方能力适配。
...
"""
    ...
    def _parse_response(self, response: str) -> TensionDiagnosis:
        """同步执行 `_parse_response` 对应的业务步骤。

        职责: 位于应用层，负责上述行为，并把相关输入、输出和副作用集中在 `_parse_response` 这一入口。
        关键输入:
        - response: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: `TensionDiagnosis`；具体语义由调用场景和返回类型共同约束。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
