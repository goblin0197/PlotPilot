...
class DatabaseConnection:
    ...
    def __init__(self, db_path: str):
        ...
        关键输入:
        - db_path: 文件系统路径，函数会据此读取、写入或定位资源。
    ...
    def _ensure_database_exists(self) -> None:
        """同步执行 `_ensure_database_exists` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_ensure_database_exists` 这一入口。
    ...
    def get_connection(self) -> sqlite3.Connection:
        """同步读取 `connection` 相关的数据或状态。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `get_connection` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
        关键输出: `sqlite3.Connection`；具体语义由调用场景和返回类型共同约束。
    ...
    def close(self) -> None:
        ...
        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `close` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
