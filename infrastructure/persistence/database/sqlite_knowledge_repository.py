...
class SqliteKnowledgeRepository:
    ...
    def save_knowledge(self, novel_id: str, premise_lock: str = "") -> None:
        """同步写入、更新或清理 `knowledge` 相关状态。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `save_knowledge` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - premise_lock: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _load_triple_children(self, novel_id:
        str) -> tuple[
        Dict[str, List[int]], Dict[str, List[str]], Dict[str, Dict[str, str]]
    ]:
        """同步执行 `_load_triple_children` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_load_triple_children` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        关键输出: `tuple[Dict[str, List[int]], Dict[str, List[str]], Dict[str, Dict[str, str]]]`；按调用约定排序或过滤后的结果集合。
        副作用: 可能读取或写入 SQLite/仓储状态。
        异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
        """
    ...
    def _load_provenance_grouped(self, novel_id: str) -> Dict[str, List[Dict[str, Any]]]:
        """同步执行 `_load_provenance_grouped` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_load_provenance_grouped` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        关键输出: `Dict[str, List[Dict[str, Any]]]`；按调用约定排序或过滤后的结果集合。
        副作用: 可能读取或写入 SQLite/仓储状态。
        异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
        """
    ...
    def _sanitize_fact_dict_for_write(triple: Dict[str, Any]) -> Dict[str, Any]:
        """同步执行 `_sanitize_fact_dict_for_write` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_sanitize_fact_dict_for_write` 这一入口。
        关键输入:
        - triple: `Dict[str, Any]` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _replace_triple_provenance(
        ...
        """同步执行 `_replace_triple_provenance` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_replace_triple_provenance` 这一入口。
        关键输入:
        - conn: 数据库连接或访问封装，用于执行持久化操作。
        - rows: 结构化数据载荷，通常来自请求、数据库行或上游服务。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 可能读取或写入 SQLite/仓储状态。
        异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
        """
    ...
    def _append_triple_provenance(
        ...
        """同步执行 `_append_triple_provenance` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_append_triple_provenance` 这一入口。
        关键输入:
        - conn: 数据库连接或访问封装，用于执行持久化操作。
        - rows: 结构化数据载荷，通常来自请求、数据库行或上游服务。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
    ...
    def get_triple_side_data_for_novel(self, novel_id:
        str) -> tuple[
        dict[str, list[int]], dict[str, list[str]], dict[str, dict[str, str]]
    ]:
    ...
    def _build_facts_from_triple_rows(
        ...
        """同步执行 `_build_facts_from_triple_rows` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_build_facts_from_triple_rows` 这一入口。
        关键输入:
        - novel_id_str: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        - triples_rows: 结构化数据载荷，通常来自请求、数据库行或上游服务。
        关键输出: `List[KnowledgeTriple]`；按调用约定排序或过滤后的结果集合。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
        if not triples_rows:
    ...
    def get_by_novel_id(self, novel_id: NovelId) -> Optional[StoryKnowledge]:
        ...
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        关键输出: `Optional[StoryKnowledge]`；当目标不存在、依赖不可用或条件不满足时可能返回 `None`。
        副作用: 可能读取或写入 SQLite/仓储状态。
        异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
        """
    ...
    def _chapter_number_from_fact(triple: Dict[str, Any]) -> Optional[int]:
        """同步执行 `_chapter_number_from_fact` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_chapter_number_from_fact` 这一入口。
        关键输入:
        - triple: `Dict[str, Any]` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _insert_triple_children(
        ...
        """同步执行 `_insert_triple_children` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_insert_triple_children` 这一入口。
        关键输入:
        - conn: 数据库连接或访问封装，用于执行持久化操作。
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - triple_id: 业务标识或配置键，用于定位目标记录。
        - triple: `Dict[str, Any]` 类型的业务输入，参与本函数的主要计算或流程控制。
        - primary_chapter: `Optional[int]` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 可能读取或写入 SQLite/仓储状态。
        异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
        """
    ...
    def _clear_triple_children(conn: sqlite3.Connection, triple_id: str) -> None:
        """同步执行 `_clear_triple_children` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_clear_triple_children` 这一入口。
        关键输入:
        - conn: 数据库连接或访问封装，用于执行持久化操作。
        - triple_id: 业务标识或配置键，用于定位目标记录。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
    ...
    def _insert_triple_row(
        ...
        """同步执行 `_insert_triple_row` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_insert_triple_row` 这一入口。
        关键输入:
        - conn: 数据库连接或访问封装，用于执行持久化操作。
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - triple: `Dict[str, Any]` 类型的业务输入，参与本函数的主要计算或流程控制。
        - now: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 可能读取或写入 SQLite/仓储状态。
        异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
        """
    ...
    def save_triple(
        ...
        *,
        ...
        """同步写入、更新或清理 `triple` 相关状态。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `save_triple` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - triple: `dict` 类型的业务输入，参与本函数的主要计算或流程控制。
        - provenance_rows: 结构化数据载荷，通常来自请求、数据库行或上游服务。
        - provenance_mode: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 可能读取或写入 SQLite/仓储状态。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
    ...
    def find_story_node_id_for_chapter_number(
        ...
        """同步读取 `story_node_id_for_chapter_number` 相关的数据或状态。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `find_story_node_id_for_chapter_number` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
    ...
    def delete_triple(self, triple_id: str) -> None:
        ...
        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `delete_triple` 这一入口。
        关键输入:
        - triple_id: 业务标识或配置键，用于定位目标记录。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 可能读取或写入 SQLite/仓储状态。
    ...
    def list_triples_by_subject(self, novel_id: str, subject: str) -> List[dict]:
        """同步读取 `triples_by_subject` 相关的数据或状态。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `list_triples_by_subject` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - subject: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def list_triples_by_predicate(self, novel_id: str, predicate: str) -> List[dict]:
        """同步读取 `triples_by_predicate` 相关的数据或状态。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `list_triples_by_predicate` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
    ...
    def list_triples_by_entity_type(self, novel_id: str, entity_type: str) -> List[dict]:
        """同步读取 `triples_by_entity_type` 相关的数据或状态。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `list_triples_by_entity_type` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
    ...
    def save_chapter_summary(self, knowledge_id: str, chapter_number: int, summary: str) -> None:
        """同步写入、更新或清理 `chapter_summary` 相关状态。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `save_chapter_summary` 这一入口。
        关键输入:
        - knowledge_id: 业务标识或配置键，用于定位目标记录。
        - chapter_number: `int` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def save(self, knowledge: StoryKnowledge) -> None:
        ...
        关键输入:
        - knowledge: `StoryKnowledge` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 可能读取或写入 SQLite/仓储状态。
        异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
        """
    ...
    def save_all(self, novel_id: str, data: dict) -> None:
        ...
        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `save_all` 这一入口。
        关键输入:
        - novel_id: 业务标识或配置键，用于定位目标记录。
