...
class SqliteChapterRepository(ChapterRepository):
    ...
    def _delete_chapter_transaction_body(
        ...
        """同步执行 `_delete_chapter_transaction_body` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_delete_chapter_transaction_body` 这一入口。
        关键输入:
        - conn: 数据库连接或访问封装，用于执行持久化操作。
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - deleted_number: `int` 类型的业务输入，参与本函数的主要计算或流程控制。
        - chapter_row_id: 业务标识或配置键，用于定位目标记录。
        - now_iso: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
    ...
    def _adjust_planning_bounds_after_delete(
        ...
        """同步执行 `_adjust_planning_bounds_after_delete` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_adjust_planning_bounds_after_delete` 这一入口。
        关键输入:
        - conn: 数据库连接或访问封装，用于执行持久化操作。
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - d: `int` 类型的业务输入，参与本函数的主要计算或流程控制。
        - now_iso: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 可能读取或写入 SQLite/仓储状态。
        异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
    ...
    def _recompute_act_chapter_bounds(
        ...
        """同步执行 `_recompute_act_chapter_bounds` 对应的业务步骤。

        职责: 位于基础设施层，负责上述行为，并把相关输入、输出和副作用集中在 `_recompute_act_chapter_bounds` 这一入口。
        关键输入:
        - conn: 数据库连接或访问封装，用于执行持久化操作。
        - novel_id: 业务标识或配置键，用于定位目标记录。
        - now_iso: `str` 类型的业务输入，参与本函数的主要计算或流程控制。
