...
class SuggestMainPlotOptionsResponse(BaseModel):
    """定义 `SuggestMainPlotOptionsResponse`，作为接口层中的Pydantic 数据模型。
...
def _storyline_to_response(storyline) -> StorylineResponse:
    """同步执行 `_storyline_to_response` 对应的业务步骤。

    职责: 位于接口层，负责上述行为，并把相关输入、输出和副作用集中在 `_storyline_to_response` 这一入口。
    关键输入:
    - storyline: 业务输入参数，参与本函数的主要计算或流程控制。
    关键输出: `StorylineResponse`；具体语义由调用场景和返回类型共同约束。
...
async def generate_chapter_stream(
    ...
    async def event_gen():
        ...
        """异步执行 `event_gen` 对应的业务步骤。

        职责: 位于接口层，负责上述行为，并把相关输入、输出和副作用集中在 `event_gen` 这一入口。
...
class GenerateBibleResponse(BaseModel):
    """定义 `GenerateBibleResponse`，作为接口层中的Pydantic 数据模型。
...
class GenerateKnowledgeResponse(BaseModel):
    """定义 `GenerateKnowledgeResponse`，作为接口层中的Pydantic 数据模型。
