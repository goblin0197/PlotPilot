...
class TensionPoint(BaseModel):
    """定义 `TensionPoint`，作为接口层中的Pydantic 数据模型。
...
class TensionCurveResponse(BaseModel):
    """定义 `TensionCurveResponse`，作为接口层中的Pydantic 数据模型。
...
class VoiceDriftResponse(BaseModel):
    """定义 `VoiceDriftResponse`，作为接口层中的Pydantic 数据模型。
...
class ForeshadowStatsResponse(BaseModel):
    """定义 `ForeshadowStatsResponse`，作为接口层中的Pydantic 数据模型。
