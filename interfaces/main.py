...
async def fix_redirect_host(request, call_next):
    """异步执行 `fix_redirect_host` 对应的业务步骤。

    职责: 位于接口层，负责上述行为，并把相关输入、输出和副作用集中在 `fix_redirect_host` 这一入口。
    关键输入:
    - request: 业务输入参数，参与本函数的主要计算或流程控制。
    - call_next: 业务输入参数，参与本函数的主要计算或流程控制。
...
def _assert_internal_shutdown_localhost(request: Request) -> None:
    """同步执行 `_assert_internal_shutdown_localhost` 对应的业务步骤。

    职责: 位于接口层，负责上述行为，并把相关输入、输出和副作用集中在 `_assert_internal_shutdown_localhost` 这一入口。
    关键输入:
    - request: `Request` 类型的业务输入，参与本函数的主要计算或流程控制。
