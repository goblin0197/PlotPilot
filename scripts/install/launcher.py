...
class BackendLauncher:
    ...
    def __init__(self, venv_py=None, on_log=None, on_progress=None,
        ...
        - on_ready: 业务输入参数，参与本函数的主要计算或流程控制。
        - on_failed: 业务输入参数，参与本函数的主要计算或流程控制。
    ...
    def _spawn_process(self, python_exe, port):
        ...
        try:
            ...
            # 后台线程：读取子进程输出并写入日志
            def _tail_backend():
                """同步执行 `_tail_backend` 对应的业务步骤。

                职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_tail_backend` 这一入口。
    ...
    def port(self):
        ...
        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `port` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
