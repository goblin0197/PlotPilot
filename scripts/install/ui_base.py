...
class SystemTray:
    ...
    def __init__(self, root_window, title="aitext", on_double_click=None):
        ...
        关键输入:
        - root_window: 业务输入参数，参与本函数的主要计算或流程控制。
        - title: 业务输入参数，参与本函数的主要计算或流程控制。
        - on_double_click: 业务输入参数，参与本函数的主要计算或流程控制。
        ...
        try:
            ...
            class NOTIFYICONDATA(ctypes.Structure):
                """定义 `NOTIFYICONDATA`，作为脚本工具层中的业务对象。
...
class BaseWindow:
    ...
    def __init__(self, title, width=WINDOW_W, height=WINDOW_H,
        ...
        关键输入:
        - title: 业务输入参数，参与本函数的主要计算或流程控制。
        - width: 业务输入参数，参与本函数的主要计算或流程控制。
        - height: 业务输入参数，参与本函数的主要计算或流程控制。
        - show_minimize: 业务输入参数，参与本函数的主要计算或流程控制。
        - on_close: 业务输入参数，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；初始化结果体现在当前实例的字段状态上。
    ...
    def _drag_start(self, e):
        """同步执行 `_drag_start` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_drag_start` 这一入口。
        关键输入:
        - e: 业务输入参数，参与本函数的主要计算或流程控制。
    ...
    def _drag_move(self, e):
        """同步执行 `_drag_move` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_drag_move` 这一入口。
        关键输入:
        - e: 业务输入参数，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
    ...
    def _on_close(self):
        """同步执行 `_on_close` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_on_close` 这一入口。
...
class StepIndicator:
    ...
    def __init__(self, parent, step_names):
        ...
        关键输入:
        - parent: 业务输入参数，参与本函数的主要计算或流程控制。
        - step_names: 业务输入参数，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；初始化结果体现在当前实例的字段状态上。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
...
class FootBar:
    ...
    def set_text(self, msg, fg=None):
        """同步写入、更新或清理 `text` 相关状态。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `set_text` 这一入口。
        关键输入:
        - msg: 业务输入参数，参与本函数的主要计算或流程控制。
        - fg: 业务输入参数，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
    ...
    def add_button(self, text, command, **opts):
        """同步执行 `add_button` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `add_button` 这一入口。
        关键输入:
        - text: 文本或提示词内容，是生成、解析或展示逻辑的主要输入。
        - command: 业务输入参数，参与本函数的主要计算或流程控制。
        - **opts: 业务输入参数，参与本函数的主要计算或流程控制。
...
def start_dot_animation(label_widget, running_ref):
    ...
    def tick():
        ...
        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `tick` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
