...
class PipInstaller:
    ...
    def __init__(self, venv_py, on_log=None, on_progress=None,
        ...
        关键输入:
        - venv_py: 业务输入参数，参与本函数的主要计算或流程控制。
        - on_log: 业务输入参数，参与本函数的主要计算或流程控制。
        - on_progress: 业务输入参数，参与本函数的主要计算或流程控制。
        - on_heartbeat: 业务输入参数，参与本函数的主要计算或流程控制。
    ...
    def _log(self, msg, tag="info"):
        ...
        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_log` 这一入口。
        关键输入:
    ...
    def _current_mirror(self):
        """同步执行 `_current_mirror` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_current_mirror` 这一入口。
    ...
    def _run_install(self, cmd, is_batch=False,
        ...
        def _heartbeat_thread():
            """同步执行 `_heartbeat_thread` 对应的业务步骤。

            职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_heartbeat_thread` 这一入口。
            关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
            关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
            副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
            异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
