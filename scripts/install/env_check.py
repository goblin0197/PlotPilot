...
class EnvChecker:
    ...
    def _show_version_warning(self, current_ver):
        ...
        try:
            ...
            def _open_download():
                """同步执行 `_open_download` 对应的业务步骤。

                职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_open_download` 这一入口。
            ...
            def _try_anyway():
                # 用户坚持要继续（不推荐但允许）
                """同步执行 `_try_anyway` 对应的业务步骤。

                职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_try_anyway` 这一入口。
                关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
                关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
                副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
                异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
