...
class HubWindow(BaseWindow):
    ...
    def __init__(self, mode="auto"):
        ...
        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `__init__` 这一入口。
        关键输入:
        - mode: 业务输入参数，参与本函数的主要计算或流程控制。
    ...
    def log(self, msg, tag="info"):
        ...
        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `log` 这一入口。
        关键输入:
        - msg: 业务输入参数，参与本函数的主要计算或流程控制。
        - tag: 业务输入参数，参与本函数的主要计算或流程控制。
    ...
    def set_progress(self, pct, label="", icon="⏳", step_idx=-1):
        """同步写入、更新或清理 `progress` 相关状态。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `set_progress` 这一入口。
        关键输入:
        - pct: 业务输入参数，参与本函数的主要计算或流程控制。
        - label: 业务输入参数，参与本函数的主要计算或流程控制。
        - icon: 业务输入参数，参与本函数的主要计算或流程控制。
        - step_idx: 业务输入参数，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
    ...
    def _open_browser(self):
        """同步执行 `_open_browser` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_open_browser` 这一入口。
    ...
    def _handle_close(self):
        """同步执行 `_handle_close` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_handle_close` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    ...
    def run(self):
        ...
        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `run` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    ...
    def _safe_exit(self):
        """同步执行 `_safe_exit` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_safe_exit` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    ...
    def _check_single_instance(self):
        """同步执行 `_check_single_instance` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_check_single_instance` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
    ...
    def _show_running_dialog(self, old_pid, old_port):
        """同步执行 `_show_running_dialog` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_show_running_dialog` 这一入口。
        关键输入:
        - old_pid: 业务输入参数，参与本函数的主要计算或流程控制。
        - old_port: 业务输入参数，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 可能访问文件系统。
        异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
        """
        ...
        def _do():
            ...
            职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_do` 这一入口。
            关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
            关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
            副作用: 可能访问文件系统。
            异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
            """
            def _open_and_close():
                """同步执行 `_open_and_close` 对应的业务步骤。

                职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_open_and_close` 这一入口。
                关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
                关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
                副作用: 可能访问文件系统。
                异常/边界: 不吞掉底层失败；数据库、文件、网络、LLM 或异步任务异常会按调用栈透传。
            ...
            def _kill_and_restart():
                """同步执行 `_kill_and_restart` 对应的业务步骤。

                职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_kill_and_restart` 这一入口。
                关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
                关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
                副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
                异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    ...
    def _launch_backend(self):
        """同步执行 `_launch_backend` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_launch_backend` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    ...
    def _show_ready_popup(self):
        ...
        def _do():
            ...
            # ── 按钮回调 ──
            def _on_minimize():
                """同步执行 `_on_minimize` 对应的业务步骤。

                职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_on_minimize` 这一入口。
                关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
                关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
                副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
                异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
            ...
            def _on_continue():
                """同步执行 `_on_continue` 对应的业务步骤。

                职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_on_continue` 这一入口。
                关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
                关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
                副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
                异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    ...
    def _start_install_extension(self):
        ...
        def _do_install():
            """同步执行 `_do_install` 对应的业务步骤。

            职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_do_install` 这一入口。
            关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
            关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
            副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
            异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
            ...
            if success:
                ...
                # 弹窗提示
                def _show_done():
                    """同步执行 `_show_done` 对应的业务步骤。

                    职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_show_done` 这一入口。
                    关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
                    关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
                    副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
                    异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
                ...
                def _show_fail():
                    """同步执行 `_show_fail` 对应的业务步骤。

                    职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_show_fail` 这一入口。
                    关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
                    关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
                    副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
                    异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    ...
    def _on_backend_failed(self, reason, detail=""):
        """同步执行 `_on_backend_failed` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_on_backend_failed` 这一入口。
        关键输入:
        - reason: 业务输入参数，参与本函数的主要计算或流程控制。
        - detail: 业务输入参数，参与本函数的主要计算或流程控制。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
        """
    ...
    def _start_pack(self):
        """同步执行 `_start_pack` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_start_pack` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    ...
    def _run_pack(self):
        """同步执行 `_run_pack` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_run_pack` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
    ...
    def _open_crash_log(self):
        """同步执行 `_open_crash_log` 对应的业务步骤。

        职责: 位于脚本工具层，负责上述行为，并把相关输入、输出和副作用集中在 `_open_crash_log` 这一入口。
        关键输入: 不接收外部业务参数，依赖当前实例状态、模块配置或调用上下文。
        关键输出: 无返回值；执行结果体现在状态变更、持久化写入、事件推送或上游调用结果中。
        副作用: 不主动修改外部状态；如传入可变对象，可能返回基于其内容计算出的结果。
        异常/边界: 通常不主动抛出业务异常；空值、缺失字段或非法输入按类型约定和上游校验处理。
