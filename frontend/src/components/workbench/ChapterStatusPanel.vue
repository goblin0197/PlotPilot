...
/**
 * 执行 `pacingLabel` 对应的前端交互逻辑。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `pacingLabel` 的状态推导、事件分发或异步调用。
...
/**
 * 执行 `qualityLabel` 对应的前端交互逻辑。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `qualityLabel` 的状态推导、事件分发或异步调用。
...
/**
 * 加载 `chapter_meta` 相关数据并同步到前端状态。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `loadChapterMeta` 的状态推导、事件分发或异步调用。
 * 关键输入输出: 主要读取当前组件的 props、ref、computed 或 store 状态。 返回值按函数签名或 Promise 约定交给调用方。
