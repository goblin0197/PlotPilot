 ...
 * 执行 `addLog` 对应的前端交互逻辑。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `addLog` 的状态推导、事件分发或异步调用。
 * 关键输入输出: `level` 表示事件载荷、当前记录或调用上下文。 `message` 表示事件载荷、当前记录或调用上下文。 返回值按函数签名或 Promise 约定交给调用方。
 * 副作用: 可能更新 Vue 响应式状态。
 ...
 * 清理 `logs` 相关的本地状态或定时器。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `clearLogs` 的状态推导、事件分发或异步调用。
...
/**
 * 切换 `auto_scroll` 对应的界面状态。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `toggleAutoScroll` 的状态推导、事件分发或异步调用。
 * 关键输入输出: 主要读取当前组件的 props、ref、computed 或 store 状态。 返回值按函数签名或 Promise 约定交给调用方。
 * 副作用: 可能更新 Vue 响应式状态。
 * 异常/边界: 缺少当前章节、slug、选中项或接口失败时按函数内的提前返回、提示或上层错误处理执行。
 */
