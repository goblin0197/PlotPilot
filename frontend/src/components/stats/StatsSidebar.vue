 ...
 * 切换 `collapse` 对应的界面状态。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `toggleCollapse` 的状态推导、事件分发或异步调用。
...
/**
 * 执行 `formatNumber` 对应的前端交互逻辑。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `formatNumber` 的状态推导、事件分发或异步调用。
...
/**
 * 加载 `stage_label` 相关数据并同步到前端状态。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `getStageLabel` 的状态推导、事件分发或异步调用。
 ...
 * 关键输入输出: `date` 表示事件载荷、当前记录或调用上下文。 返回值按函数签名或 Promise 约定交给调用方。
 * 副作用: 不主动修改外部状态，主要返回计算结果或整理后的数据。
 * 异常/边界: 缺少当前章节、slug、选中项或接口失败时按函数内的提前返回、提示或上层错误处理执行。
 */
