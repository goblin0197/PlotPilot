...
/**
 * 执行 `parseSseDataLine` 对应的前端交互逻辑。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `parseSseDataLine` 的状态推导、事件分发或异步调用。
 * 关键输入输出: `line` 表示事件载荷、当前记录或调用上下文。 返回值按函数签名或 Promise 约定交给调用方。
...
/**
 * 执行 `retrieveContext` 对应的前端交互逻辑。
 *
 * 职责: 位于前端组件/组合式逻辑中，集中处理 `retrieveContext` 的状态推导、事件分发或异步调用。
