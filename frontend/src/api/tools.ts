...
export const narrativeStateApi = {
  /**
   * 加载 `state` 相关数据并同步到前端状态。
   *
   * 职责: 位于前端组件/组合式逻辑中，集中处理 `getState` 的状态推导、事件分发或异步调用。
   * 关键输入输出: `novelId` 表示事件载荷、当前记录或调用上下文。 `entityId` 表示事件载荷、当前记录或调用上下文。 `chapter` 表示事件载荷、当前记录或调用上下文。 返回值按函数签名或 Promise 约定交给调用方。
