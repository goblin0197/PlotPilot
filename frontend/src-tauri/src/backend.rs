...
impl BackendManager {
    /// 创建后端进程管理器，解析项目根目录并初始化端口、子进程与 Windows Job 状态。
    /// 该函数只保存 Tauri `AppHandle` 和初始管理状态，实际启动后端由后续启动方法完成。
    ...
    /// 按发布/调试策略向后端启动命令注入生产数据目录和日志文件路径。
    /// 该函数会创建数据与日志目录；路径解析或目录创建失败时以 `String` 错误返回。
    ...
    /// 强制结束当前受管后端进程，并在 Windows 下释放 Job Object 以清理子进程树。
    /// 该兜底路径用于退出、重启或正常终止失败后的清理，不向调用方返回错误。
...
impl Drop for BackendManager {
    /// 析构后端管理器时兜底终止后端进程，避免桌面端退出后留下后台进程。
    /// 这里不传播错误，清理失败只会保留在底层日志或操作系统状态中。
    fn drop(&mut self) {
        self.terminate_hard();
    }
}
...
#[cfg(target_os = "windows")]
/// 返回 Windows 隐藏子进程控制台窗口所需的 `CREATE_NO_WINDOW` 标志。
/// 该值只影响启动命令的窗口行为，不改变后端进程的业务参数。
...
/// 非 Windows 平台不需要隐藏控制台窗口，返回 `0` 作为无操作标志。
/// 该函数保持跨平台调用点一致，避免调用方到处分支判断平台。
fn windows_subsystem_flag() -> u32 {
    0
}
